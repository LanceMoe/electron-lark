...
let updateCheckJson: any;

function getConfigJson(callback: () => void) {
...
function saveConfig() {
  ...
  appStoreFs.writeFile(
    'updateCheck.json',
    JSON.stringify(updateCheckJson, null, '  '),
    function (err) {
      if (err) {
        console.log(err);
      } else {
...
// 参数代表是否手动触发
function checkUpdate(showCheckNotify?: boolean, successCb?: () => void) {
  ...
  https
    .get(options, (res) => {
      ...
      res.on('end', () => {
        ...
        const newVersion = String(resJson.tag_name).trim().toLocaleLowerCase().replace('v', '');

        // 只要最新版本和当前版本不一样，就提示更新
        const packageVersion = version.trim().toLocaleLowerCase().replace('v', '');
