...
import rightClickContextMenu from 'electron-context-menu';
...
let newAppTray: electron.Tray = null;
...
type WindowConfigProps = {
  ...
  showWarterMark?: boolean;
  larkOpenLink?: string;
};

function createWindow(configJson?: WindowConfigProps) {
  ...
  if (
    configJson.startPageLink !== undefined &&
  ...
  if (configJson.showWarterMark !== undefined && configJson.showWarterMark != null) {
  ...
  webContents.on('did-finish-load', function () {
    setInterval(() => {
      ...
      webContents
        .executeJavaScript(
          'document.getElementsByClassName(\'larkc-badge-count circle larkc-badge-normal\').length',
        )
      ...
      if (!showWarterMark) {
        webContents.executeJavaScript(
          'if(document.getElementsByClassName(\'lark-water-mark-main\').length > 0) document.getElementsByClassName(\'lark-water-mark-main\')[0].remove()',
        );
      }
    }, 1500);
  ...
  webContents.on(
    'new-window',
    function (event, url, frameName, disposition, options, features, referer) {
...
let blingCount = 0;
let blingTimer: NodeJS.Timer = null;
let bling = false;
...
function getConfigJson(callback: (configJson: WindowConfigProps) => void) {
...
function showLarkNotify(title: string, opt: Record<string, string>, channelId: string) {
